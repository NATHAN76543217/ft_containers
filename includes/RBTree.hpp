...
class RBTree {
		...
		node	&insert(const value_type &new_val)
		{
			...
			while (current != nullptr)
			{
				if (new_val < current->value)
				{
					...
					{
						current->left = this->create_node(new_val, current);
				...
				else if (new_val > current->value)
				{
					...
					{
						current->right = this->create_node(new_val, current);
	...
	protected:
		...
		void	fix_insert(node *k)
		{
			...
			{
				std::cout << "INSERT(" << k->value << "): P is red and" << std::endl;
				node * uncle = k->getUncle();
				if (uncle == nullptr)
					std::cout << "uncle(null)" << std::endl;
				else
					std::cout << "uncle(" << uncle->value << ")" << std::endl;
				if (uncle != nullptr && uncle->color == RED)
				{
					std::cout << "       : U is red" << std::endl;
					uncle->swapColor();
				...
				{
					std::cout << "      : U is black and" << std::endl;
					if (k->parent->isRightChild()) //TODO factorise
					{
						std::cout << "      : P is right and" << std::endl;
						if (!k->isRightChild())
						{
							std::cout << "      : K is left" << std::endl;
							k = k->parent; 
							this->rightRotate(k);
						}else
							std::cout << "      : K is right" << std::endl;
						//TODO handle the three other cases
						k->parent->swapColor();
						k->parent->parent->swapColor();
						this->leftRotate(k->parent->parent);
					} else {
						std::cout << "      : P is left and" << std::endl;
						if (k->isRightChild())
						{
							std::cout << "      : K is right" << std::endl;
							k = k->parent;
						...
						// std::cout << "Before  rotate G = " << current->parent->parent << " and S = " << node::getSibling(*current) << std::endl;
						k->parent->swapColor();
						k->parent->parent->swapColor();
						this->rightRotate(k->parent->parent);
						// std::cout << "after left rotate G = " << current->parent->parent << " and S = " << S << std::endl;
					}
				}
				if (k == this->_root) {
					break ;
		...
		void	leftRotate(node *x)
		{
			...
			if (x->parent == nullptr)
				...
				x->parent->left = y;
			x->parent = y;
			//TODO left rotate and right rotate
		}
